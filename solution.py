...
class SOLUTION:
    ...
    def Create_Body(self):
        ...
        pyrosim.Send_Cube(name="Torso", pos=[0,0,1] , size=[2.25,1,0.5], mass = 3)

#NECK
        pyrosim.Send_Joint( name = "Torso_Neck" , parent= "Torso" , child = "Neck" , type = "revolute", position = [0.75,0,1], jointAxis = "0 1 0")
        pyrosim.Send_Cube(name="Neck", pos=[0.25,0,0.5] , size=[0.2,0.2,0.5], mass = 0.5)

#NEWLY ADDED HEAD
        pyrosim.Send_Joint( name = "Neck_Head" , parent= "Neck" , child = "Head" , type = "revolute", position = [0.25,0,0], jointAxis = "0 1 0")
        pyrosim.Send_Cube(name="Head", pos=[0.25,0,1] , size=[1,0.5,0.5], mass = 0.5)

        pyrosim.Send_Joint( name = "Torso_Thigh" , parent= "Torso" , child = "Thigh" , type = "revolute", position = [0,-0.2,1], jointAxis = "0 1 0")
        pyrosim.Send_Cube(name="Thigh", pos=[-0.75,-0.5,0] , size=[0.2,0.5,0.2], mass = 0.1)


        pyrosim.Send_Joint( name = "Thigh_BackLeg" , parent= "Thigh" , child = "BackLeg" , type = "revolute", position = [-0.5,-0.5,0.5], jointAxis = "0 1 0")
        pyrosim.Send_Cube(name="BackLeg", pos=[-0.75,0,-0.5] , size=[1,0.2,0.2], mass = 0.5)
        
        #back leg lower
        pyrosim.Send_Joint( name = "BackLeg_BackLegLow" , parent= "BackLeg" , child = "BackLegLow" , type = "revolute", position = [-1,0,0], jointAxis = "0 1 0")
        pyrosim.Send_Cube(name="BackLegLow", pos=[-0.25,0,-1] , size=[0.2,0.2,1],  mass = 0.5)


        #front 
        #[foward(pos)/backward(neg), left(pos)/right(neg), up(pos)/down(neg)]
        #Front Thigh
        pyrosim.Send_Joint( name = "Torso_ThighFront" , parent= "Torso" , child = "ThighFront" , type = "revolute", position = [0,0.2,1], jointAxis = "0 1 0")

        pyrosim.Send_Cube(name="ThighFront", pos=[.75,0.5,0] , size=[0.2,0.5,0.2], mass = 0.1)

        pyrosim.Send_Joint( name = "ThighFront_FrontLeg" , parent= "ThighFront" , child = "FrontLeg" , type = "revolute", position = [0,0.5,0.5], jointAxis = "1 0 0")

        pyrosim.Send_Cube(name="FrontLeg", pos=[0.25,0,-0.5] , size=[1,0.2,0.2], mass = 0.5)
        #front lower 

        pyrosim.Send_Joint( name = "FrontLeg_FrontLegLow" , parent= "FrontLeg" , child = "FrontLegLow" , type = "revolute", position = [-0.5,0,0], jointAxis = "0 1 0")

        pyrosim.Send_Cube(name="FrontLegLow", pos=[0.25,0,-1] , size=[0.2,0.2,1],  mass = 0.5)  
        #left

        pyrosim.Send_Joint( name = "Torso_LeftLeg" , parent= "Torso" , child = "LeftLeg" , type = "revolute", position = [0,-0.5,1], jointAxis = "1 0 0")

        pyrosim.Send_Cube(name="LeftLeg", pos=[0.75,-0.5,0] , size=[0.2,1,0.2], mass = 0.5)

        #lower
        pyrosim.Send_Joint( name = "LeftLeg_LeftLegLow" , parent= "LeftLeg" , child = "LeftLegLow" , type = "revolute", position = [0,-1,0], jointAxis = "0 1 0")
        pyrosim.Send_Cube(name="LeftLegLow", pos=[0.75,0,-0.5] , size=[0.2,0.2,1], mass = 0.5)

        #right
        pyrosim.Send_Joint( name = "Torso_RightLeg" , parent= "Torso" , child = "RightLeg" , type = "revolute", position = [0,0.5,1], jointAxis = "1 0 0")
        pyrosim.Send_Cube(name="RightLeg", pos=[-0.75,0.5,0] , size=[0.2,1,0.2], mass = 0.5)

        #right leg low
        pyrosim.Send_Joint( name = "RightLeg_RightLegLow" , parent= "RightLeg" , child = "RightLegLow" , type = "revolute", position = [0,1,0], jointAxis = "0 1 0")
        pyrosim.Send_Cube(name="RightLegLow", pos=[-0.75,0,-0.5] , size=[0.2,0.2,1],  mass = 0.5)
    ...
    def Create_Brain(self):
        ...
        pyrosim.Send_Sensor_Neuron(name = 19 , linkName = "Thigh")
        pyrosim.Send_Motor_Neuron( name = 20 , jointName = "Torso_Thigh")
        pyrosim.Send_Sensor_Neuron(name = 19 , linkName = "ThighFront")
        pyrosim.Send_Motor_Neuron( name = 20 , jointName = "Torso_ThighFront")
